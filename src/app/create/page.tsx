...
function CreateBingoContent() {
      ...
      if (currentStep === 2 && !tempBingoId && !isEditMode) {
        try {
          const tempBingo = await bingoService.create({
            ...formData,
            name: formData.name || "Bingo en cours de création",
          });
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      const updatedBingo = await bingoService.update(tempBingoId, formData);
      ...
      // Generate grids
      const gridsToCreate = [];
      for (let i = 0; i < formData.player_count; i++) {
        gridsToCreate.push({
          name: `Grille ${i + 1}`,
      ...
      <div className="max-w-[1600px] mx-auto px-6 py-6">
        ...
        <div className="text-center mb-6">
          <h1 className="text-2xl md:text-3xl font-bold gradient-text">
            {isEditMode ? `Modifier : ${formData.name}` : "Créer un nouveau bingo"}
          </h1>
        </div>
